...
const DEFAULT_MODS = {
  'Chin-ups': {
    ...
    progression:
      'Add 1 unassisted rep per cycle. If 0 unassisted: jumping negatives 3×8. 1–3: mixed sets. 4+: full sets.',
  },
};
...
const LIFT_DETAILS = {
  OHP: {
    ...
    mcgill: true,
    accessory: [
      { name: 'Chin-ups', prescribed: '3×8–10', mod: true },
      { name: 'DB Lateral Raise', prescribed: '3×15', mod: false },
      { name: 'Tricep Pushdown', prescribed: '3×12', mod: false },
    ],
  },
  Deadlift: {
    tag: 'Hinge',
    mcgill: true,
    accessory: [
      { name: 'Romanian Deadlift', prescribed: '3×10 @40% TM', mod: false },
      { name: 'Plank', prescribed: '3×60s', mod: false },
      { name: 'Face Pull', prescribed: '3×20', mod: false },
    ],
  },
  Bench: {
    tag: 'Press',
    mcgill: true,
    accessory: [
      { name: 'Barbell Row', prescribed: '3×10', mod: false },
      { name: 'DB Curl', prescribed: '3×12', mod: false },
      {
        name: 'Ab Wheel / Hollow Body',
        prescribed: '3×10 / 3×30s',
        mod: false,
      },
    ],
  },
  Squat: {
    ...
    mcgill: false,
    accessory: [
      { name: 'Hip Thrust', prescribed: '3×12', mod: false },
      { name: 'Nordic Curl / Leg Curl', prescribed: '3×8', mod: false },
      { name: 'Calf Raise', prescribed: '3×15', mod: false },
    ],
  },
};
    ...
    if (LIFT_WEEKDAYS.includes(prev.getDay())) {
      ...
      if (res)
        return {
          ...
          isLiftDay: false,
          actualDate: prevStr,
          todayIsRunDay: RUN_WEEKDAYS.includes(
            new Date(targetStr + 'T12:00:00').getDay()
          ),
          todayIsRest: !RUN_WEEKDAYS.includes(
            new Date(targetStr + 'T12:00:00').getDay()
          ),
        };
      break;
...
function SetLogger({
  ...
  const sets = logs[key] || [];
  function addSet() {
    setLogs({ ...logs, [key]: [...sets, { weight: '', reps: '', note: '' }] });
  }
  function removeSet(i) {
    setLogs({ ...logs, [key]: sets.filter((_, si) => si !== i) });
  }
  function updateSet(i, field, val) {
    setLogs({
      ...logs,
      [key]: sets.map((s, si) => (si === i ? { ...s, [field]: val } : s)),
    });
  }
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {sets.map((s, i) => (
        ...
        >
          <span style={{ fontSize: 11, color: MUTED, minWidth: 20 }}>
            S{i + 1}
          </span>
          {!isTime && (
            <Input
          ...
          <Input
            ...
            placeholder={isTime ? 'secs' : 'reps'}
            style={{ width: 60 }}
          />
          <Input
            value={s.note}
            onChange={(v) => updateSet(i, 'note', v)}
            placeholder="note"
            type="text"
            style={{ flex: 1 }}
          />
      ...
      {open && (
        <div style={{ padding: '0 12px 12px', fontSize: 12, lineHeight: 1.7 }}>
          <div style={{ color: MUTED }}>
            <span style={{ color: TEXT, fontWeight: 700 }}>Now: </span>
            {mod.current}
          </div>
          <div style={{ color: MUTED }}>
            <span style={{ color: GREEN, fontWeight: 700 }}>Target: </span>
            {mod.target}
          </div>
          <div style={{ color: MUTED }}>
            <span style={{ color: BLUE, fontWeight: 700 }}>Progression: </span>
            {mod.progression}
          </div>
        </div>
      )}
...
function SessionLogger({
  ...
  const resolved = startDate
    ? resolveSession(startDate, baseSessionIdx, baseWeekIdx, todayStr)
    : null;
  ...
  const isRestDay =
    !LIFT_WEEKDAYS.includes(todayDow) && !RUN_WEEKDAYS.includes(todayDow);
  ...
  function saveSession() {
    const entry = {
      ...
      logs: Object.fromEntries(
        Object.entries(logs).filter(([k]) => k.startsWith(sessionKey))
      ),
      note: sessionNote,
    };
    ...
    const idx = trimmed.findIndex(
      (s) => s.date === showDate && s.lift === lift
    );
    const next =
      idx >= 0
        ? trimmed.map((s, i) => (i === idx ? entry : s))
        : [...trimmed, entry];
    ...
    <Card>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: MUTED }}>
          {isLiftDay
            ...
            : `Last lift: ${dayName(showDate)} · Today is a ${
                isRunDay ? 'run day 🏃' : 'rest day'
              }`}
          {nextLift && !isLiftDay && (
            <span style={{ color: ACCENT, marginLeft: 10 }}>
              Next lift: {dayName(nextLift)} —{' '}
              {LIFT_ORDER[(sessionIdx + 1) % 4]}
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <button
          ...
          style={{
            background: 'none',
            border: `1px solid ${showOverride ? ACCENT : BORDER}`,
            borderRadius: 6,
            padding: '5px 14px',
            fontSize: 11,
            fontWeight: 700,
            color: showOverride ? ACCENT : MUTED,
            cursor: 'pointer',
        ...
        {showOverride && (
          ...
          >
            <div style={{ flex: 1, minWidth: 140 }}>
              ...
              <select
                value={overrideLift}
                onChange={(e) => setOverrideLift(e.target.value)}
                style={{
                  width: '100%',
                  background: SURFACE2,
                  border: `1px solid ${BORDER}`,
      ...
      {!isLiftDay && (
        <div
          style={{
            background: isRunDay ? `${BLUE}15` : SURFACE2,
            border: `1px solid ${isRunDay ? BLUE : BORDER}`,
            borderRadius: 8,
            padding: '10px 14px',
            marginBottom: 16,
            fontSize: 13,
        ...
        >
          {isRunDay ? (
            <span style={{ color: BLUE }}>
              🏃 <strong>Run day</strong> — Coach Greg 10K Plan. Showing last
              lift session below for reference.
            </span>
          ) : (
            <span style={{ color: MUTED }}>
              😴 <strong>Rest day</strong> — Showing last lift session below for
              reference.
            </span>
          )}
        </div>
      ...
      {scheme.sets.map((s, si) => {
        const label = prescribed
          ? `${prescribed[si].weight} lbs × ${
              typeof s.reps === 'string' ? s.reps : s.reps
            }${si === 2 && !isDeload ? ' (AMRAP)' : ''}`
          : `${Math.round(s.pct * 100)}% TM × ${s.reps}`;
        return (
          <SetLogger
            key={`main-${si}`}
            exerciseName={`Main Set ${si + 1}`}
            prescribed={label}
      ...
      {!isDeload && (
        <>
          ...
          {[0, 1, 2].map((i) => (
            <SetLogger
              key={`fsl-${i}`}
              exerciseName={`FSL Set ${i + 1}`}
              prescribed={
                fslWeight
                  ? `${fslWeight} lbs × 5`
                  : `${Math.round(scheme.sets[0].pct * 100)}% TM × 5`
              }
              sessionKey={sessionKey}
              logs={logs}
              setLogs={setLogs}
      ...
      <button
        ...
        style={{
          marginTop: 12,
          background: saveBounce ? GREEN : ACCENT,
          color: '#0F1114',
          border: 'none',
          borderRadius: 6,
          padding: '12px 28px',
...
function TMSetup({
  ...
  const [inputs, setInputs] = useState(
    Object.fromEntries(
      LIFTS.map((l) => [l, tms[l] ? Math.round(tms[l] / 0.9) : ''])
    )
  );
  ...
  const allFilled = LIFTS.every(
    (l) => inputs[l] && !isNaN(parseFloat(inputs[l]))
  );

  function handleApply() {
    const next = {};
    LIFTS.forEach((l) => {
      const r = parseFloat(inputs[l]);
      if (!isNaN(r) && r > 0) next[l] = round5(r * 0.9);
    });
  ...
  return (
    <Card>
      ...
      >
        Set your 1RMs, program start date, and where you are in the cycle. The
        app uses the start date to automatically show the right lift each day.
      </p>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 11, color: MUTED, marginTop: 6 }}>
          Set this to the Monday you begin Week 1. The app counts lift days from
          here.
        </div>
      </div>
...
function WeekOverview({ startDate, baseSessionIdx, baseWeekIdx }) {
  ...
  function getLift(offset) {
    return LIFT_ORDER[(sessionIdx + offset) % 4];
  }
  const liftDays = [
    { day: 'Monday', lift: getLift(0) },
    { day: 'Wednesday', lift: getLift(1) },
    { day: 'Friday', lift: getLift(2) },
  ];
  ...
  return (
    <Card>
        ...
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((d, i) => {
          ...
          return (
            <div
              key={d}
              style={{
                background: isToday ? `${ACCENT}20` : SURFACE2,
                border: `1px solid ${
                  isToday ? ACCENT : isRun ? `${BLUE}40` : BORDER
                }`,
                borderRadius: 6,
                padding: '8px 4px',
                textAlign: 'center',
            ...
            >
              ...
              {!isLift && !isRun && (
                <div style={{ fontSize: 10, color: MUTED }}>Rest</div>
              )}
            </div>
          );
        })}
        ...
        {[0, 1, 2, 3].map((wo) => {
          ...
          return (
            <div
              key={wo}
              style={{
                background: wo === 0 ? `${ACCENT}15` : SURFACE2,
                border: `1px solid ${wo === 0 ? ACCENT : BORDER}`,
                borderRadius: 7,
                padding: '10px 12px',
              }}
            >
              <div
                style={{
                  fontSize: 10,
                  color: wo === 0 ? ACCENT : MUTED,
                  fontWeight: 700,
                  letterSpacing: '0.1em',
                  marginBottom: 6,
                }}
              >
                {wo === 0 ? 'THIS WEEK' : `WEEK +${wo}`}
              </div>
              {[
                ['Mon', mon],
                ['Wed', wed],
                ...
                <div
                  ...
                  style={{
                    ...
                    justifyContent: 'space-between',
                    padding: '2px 0',
                    color: l === 'Squat' ? GREEN : TEXT,
                  }}
                >
                  <span style={{ color: MUTED }}>{d}</span>
                  <span style={{ fontWeight: l === 'Squat' ? 700 : 400 }}>
                    {l}
                  </span>
                </div>
              ))}
      ...
      >
        <span style={{ color: ACCENT, fontWeight: 700 }}>⚠ Watch: </span>Wed
        Deadlift → Thu Run. Monitor for 2 weeks before adjusting.
      </div>
    </Card>
  );
}
...
function CycleTracker({ tms, setTms, cycle, setCycle }) {
  ...
  const [tmInputs, setTmInputs] = useState(
    Object.fromEntries(LIFTS.map((l) => [l, tms[l] || '']))
  );
  ...
  return (
    <Card>
      ...
      >
        Only advance after the deload week. Hold TM flat on a cut if AMRAP felt
        like a grind. Use <strong style={{ color: ACCENT }}>Edit TMs</strong>{' '}
        anytime to manually adjust — mid-cycle resets, failed reps, or
        corrections.
      </p>
      ...
      >
        {LIFTS.map((l) => (
          <div
            ...
            style={{
              background: SURFACE2,
              border: `1px solid ${editingTMs ? ACCENT : BORDER}`,
              borderRadius: 8,
              padding: '14px 16px',
            }}
          >
            ...
            {editingTMs ? (
              <>
                <input
                  ...
                  value={tmInputs[l]}
                  onChange={(e) =>
                    setTmInputs((p) => ({ ...p, [l]: e.target.value }))
                  }
                  placeholder="lbs TM"
                  style={{
                    width: '100%',
                ...
                <div style={{ fontSize: 10, color: MUTED, marginTop: 4 }}>
                  ≈{' '}
                  {tmInputs[l] && !isNaN(parseFloat(tmInputs[l]))
                    ? Math.round(parseFloat(tmInputs[l]) / 0.9)
                    : '—'}{' '}
                  lbs 1RM
                </div>
              </>
      ...
      <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 20 }}>
        ...
        <button
          ...
          disabled={!hasTMs}
          style={{
            background: hasTMs ? GREEN : BORDER,
            color: hasTMs ? '#0F1114' : MUTED,
            border: 'none',
            borderRadius: 6,
            padding: '12px 28px',
            fontWeight: 700,
            fontSize: 14,
            cursor: hasTMs ? 'pointer' : 'default',
          }}
        >
          Complete Deload → Advance to Cycle {cycle + 1}
        </button>
        <div style={{ fontSize: 11, color: MUTED, marginTop: 8 }}>
          Advances all TMs by standard increments: OHP +5, Bench +5, Deadlift
          +10, Squat +10
        </div>
      </div>
    </Card>
...
function ModsManager({ mods, setMods }) {
  ...
  function startEdit(name) {
    setEditing(name);
    setForm(mods[name] || { current: '', target: '', progression: '' });
  }
  function saveEdit() {
    setMods({ ...mods, [editing]: form });
    setEditing(null);
  }
  function addNew() {
    ...
    setMods({
      ...mods,
      [newName.trim()]: { current: '', target: '', progression: '' },
    });
    setNewName('');
  }
  ...
  return (
    <Card>
      ...
      >
        Track scaled exercises and progression targets. These appear as
        expandable cards in Today's session.
      </p>
  ...
  if (sessionHistory.length === 0)
    return (
      <Card>
        ...
        <p style={{ color: MUTED, fontSize: 13 }}>No sessions logged yet.</p>
      </Card>
    );
      ...
      {sorted.map((session, i) => (
        <div
          ...
          style={{
            ...
            paddingBottom: 20,
            borderBottom:
              i < sorted.length - 1 ? `1px solid ${BORDER}` : 'none',
          }}
        >
          <div
            style={{
...
${
  hasTMs
    ? LIFTS.map(
        (l) => `- ${l}: ${tms[l]} lbs TM (≈${Math.round(tms[l] / 0.9)} lbs 1RM)`
      ).join('\n')
    : 'Not set'
}
...
2. Set rotation position to ${sessionIdx} and week to "${
    WEEK_SCHEMES[weekIdx]?.label
  }"
3. Enter TMs above
`;
  ...
  return (
    <Card>
      ...
      >
        Full snapshot including TMs, modifications, session logs, and restore
        instructions. Paste into Notion after every deload.
      </p>
      <pre
        style={{
          background: SURFACE2,
      ...
      <button
        onClick={handleCopy}
        style={{
          background: copied ? `${GREEN}20` : ACCENT,
          border: `1px solid ${copied ? GREEN : ACCENT}`,
          borderRadius: 6,
          padding: '12px 28px',
          fontWeight: 700,
          fontSize: 14,
          color: copied ? GREEN : '#0F1114',
          cursor: 'pointer',
        }}
      >
...
function ProgramNotes() {
  const notes = [
    {
      title: 'Why FSL instead of BBB',
      color: ACCENT,
      body: "BBB's 5×10 is contraindicated on a caloric deficit. FSL (3×5 at your lightest working weight) maintains volume without wrecking recovery. Switch to BBB when you hit maintenance near 160 lbs.",
    },
    {
      title: '3-day rotation',
      color: BLUE,
      body: 'OHP → Deadlift → Bench → Squat rotates across Mon/Wed/Fri continuously. Each lift hits every ~8 days. A full 5/3/1 cycle takes ~5 weeks on this split — normal and Wendler-endorsed.',
    },
    {
      title: 'Cut-phase expectations',
      color: BLUE,
      body: "Strength will feel flat. Don't chase PRs — chase quality reps. Hold TM flat an extra cycle if AMRAP sets felt like a true grind.",
    },
    {
      title: 'AFT maintenance',
      color: GREEN,
      body: 'OHP + chin-ups → HRP. Deadlift + plank → Deadlift + Plank events. Coach Greg 10K builds more aerobic base than a straight 2-mile plan — your 17:13 is protected and should improve.',
    },
    {
      title: 'McGill Big 3',
      color: MUTED,
      body: 'Bird-dog, curl-up, side plank — pre-lift on OHP/Deadlift/Bench days. Non-negotiable.',
    },
    {
      title: 'Wed → Thu flag',
      color: ACCENT,
      body: 'Deadlift day followed by a run day. Monitor the first 2 weeks. Swap if consistently problematic.',
    },
  ];
  return (
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {notes.map((n, i) => (
          <div
            key={i}
            style={{ borderLeft: `3px solid ${n.color}`, paddingLeft: 16 }}
          >
            <div
              style={{
                fontSize: 13,
...
export default function App() {
  ...
  const [baseSessionIdx, setBaseSessionIdxRaw] = useState(() =>
    load('of_session', 0)
  );
  ...
  function setTms(v) {
    const val = typeof v === 'function' ? v(tms) : v;
    save('of_tms', val);
    setTmsRaw(val);
  }
  ...
  function setBaseSessionIdx(v) {
    const val = typeof v === 'function' ? v(baseSessionIdx) : v;
    save('of_session', val);
    setBaseSessionIdxRaw(val);
  }
  function setBaseWeekIdx(v) {
    const val = typeof v === 'function' ? v(baseWeekIdx) : v;
    save('of_week', val);
    setBaseWeekIdxRaw(val);
  }
  function setCycle(v) {
    const val = typeof v === 'function' ? v(cycle) : v;
    save('of_cycle', val);
    setCycleRaw(val);
  }
  function setLogs(v) {
    const val = typeof v === 'function' ? v(logs) : v;
    save('of_logs', val);
    setLogsRaw(val);
  }
  function setSessionHistory(v) {
    const val = typeof v === 'function' ? v(sessionHistory) : v;
    save('of_history', val);
    setSessionHistoryRaw(val);
  }
  function setMods(v) {
    const val = typeof v === 'function' ? v(mods) : v;
    save('of_mods', val);
    setModsRaw(val);
  }

  useEffect(() => {
    if (Object.keys(tms).length > 0 && startDate) {
      setRestored(true);
  ...
  const todayDow = new Date(todayStr + 'T12:00:00').getDay();
  const todayType = LIFT_WEEKDAYS.includes(todayDow)
    ? 'Lift'
    : RUN_WEEKDAYS.includes(todayDow)
    ? 'Run'
    : 'Rest';
        ...
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
              ...
              >
                5/3/1 FSL · COACH GREG 10K
              </span>
            </div>
            <button
              onClick={copyUrl}
              style={{
                background: urlCopied ? `${GREEN}20` : SURFACE2,
                border: `1px solid ${urlCopied ? GREEN : BORDER}`,
                borderRadius: 5,
                padding: '4px 12px',
                fontSize: 11,
                fontWeight: 700,
                color: urlCopied ? GREEN : MUTED,
                cursor: 'pointer',
                textTransform: 'uppercase',
                letterSpacing: '0.08em',
              }}
          ...
          <p style={{ margin: '2px 0 14px', fontSize: 12, color: MUTED }}>
            {isSetup ? (
              ...
              <span>
                ...
                {restored && (
                  <span
                    style={{ color: GREEN, marginLeft: 10, fontWeight: 700 }}
                  >
                    ✓ Restored
                  </span>
                )}
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 16px 0' }}>
        ...
        {tab === 'session' && (
          <SessionLogger
            tms={tms}
            startDate={startDate}
            baseSessionIdx={baseSessionIdx}
            baseWeekIdx={baseWeekIdx}
            logs={logs}
            setLogs={setLogs}
        ...
        {tab === 'setup' && (
          <TMSetup
            ...
            baseSessionIdx={baseSessionIdx}
            setBaseSessionIdx={setBaseSessionIdx}
            baseWeekIdx={baseWeekIdx}
            setBaseWeekIdx={setBaseWeekIdx}
          />
        )}
        {tab === 'export' && (
          <ExportSnapshot
